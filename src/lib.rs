...
pub fn say_hello() -> Option<RawBytes> {
    ...
    // let x = sdk::actor::get_code_cid_for_type(3);
    let y = sdk::actor::get_code_cid_for_type(Type::Miner);
    let addr = sdk::actor::resolve_builtin_actor_type(&y).unwrap();

    let ret = to_vec(format!("Hello world #{} and {}!", &state.count, &addr).as_str());
    match ret {
